 ...
 */
function wc_hyba_gateways_get_order_id( $order ) {
 ...
 */
function wc_hyba_gateways_get_customer_billing_country() {
 ...
 * @return string Country
 */
function wc_hyba_gateways_get_customer_ip_address( $order ) {
 ...
 */
function wc_hyba_gateways_get_order_total( $order ) {
 ...
 */
function wc_hyba_gateways_get_order_billing_email( $order ) {
...
/**
 * Get order billing phone based on WC version
 *
 * @since  1.4
 * @param  WC_Order $order Order
 * @return string          Order billing phone
 */
function wc_hyba_gateways_get_order_billing_phone( $order ) {
  if( method_exists( $order, 'get_billing_phone' ) ) {
		return validateCountryPrefix($order->get_billing_phone());
	}
	else {
		return validateCountryPrefix($order->billing_phone);
	}
}


function validateCountryPrefix($phone){

    if(empty($phone) || is_null($phone)){
        return "";
    }

    if(substr( $phone, 0, 4 ) == "+372"){
        return $phone;
    }else{
 ...
 * @return integer         Order ID
 */
function wc_hyba_gateways_get_order_currency( $order ) {
